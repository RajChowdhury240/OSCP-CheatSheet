...
if [[ ${result} == "$SAFE_KERNEL" ]]; then
    ...
    echo -e "It is recommended that you update your kernel as soon as possible."
    EXITCODE=2
else
    echo -e "${RED}Your kernel is ${RESET}$running_kernel${RED} which IS vulnerable.${RESET}"
    echo -e "It is recommends that you update your kernel. Alternatively, you can apply partial"
...
if [[ -n $vulnerable_update_kernel ]]; then
    echo -e "${RED}The kernel available to update to is ${RESET}$UPDATE_VERSION${RED} IS also vulnerable.${RESET}"
fi
